...
<section class="single">
  ...
  <div class="content">
          ...
          {#each data.whatWeDo[0]?.credits as credit, j}
            {#if credit._type === 'partners' && credit.partnersArray?.length}
              ...
              {#each credit.partnersArray as partner, k}
                {#if partner.link}
                  <a target="_blank" href={partner.link}><p class="underline">{partner.title}</p></a>
                {:else}
                  <p>{partner?.title}</p>
                {/if}
              {/each}
            {:else if credit._type === 'sponsors' && credit.sponsorsArray?.length}
              ...
              {#each credit.sponsorsArray as sponsor, k}
                {#if sponsor.link}
                  <a target="_blank" href={sponsor.link}><p class="underline">{sponsor.title}</p></a>
                {:else}
                  <p>{sponsor?.title}</p>
                {/if}
              {/each}
            {:else if credit._type === 'patronages' && credit.patronagesArray?.length}
              ...
              {#each credit.patronagesArray as patronage, k}
                {#if patronage.link}
                  <a target="_blank" href={patronage.link}><p class="underline">{patronage.title}</p></a>
                {:else}
                  <p>{patronage?.title}</p>
                {/if}
              {/each}
            {:else if credit._type === 'customCredit' && credit.customsArray?.length}
              ...
              {#each credit.customsArray as custom, k}
                {#if custom.link}
                  <a target="_blank" href={custom.link}><p class="underline">{custom.title}</p></a>
                {:else}
                  <p>{custom?.title}</p>
                {/if}
              {/each}
    ...
    src={urlFor(data.whatWeDo[0].image ? data.whatWeDo[0].image : data.whatWeDo[0].thumbnail)} alt=""
    >
  </div>
</section>
