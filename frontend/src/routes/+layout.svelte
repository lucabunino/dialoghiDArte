...
<!-- {#if $page.url.host === 'localhost:5173' || $page.url.host === 'dialoghi-d-arte.vercel.app'} -->
  <header class:up={scrolledDown} class:down={showMenu}>
    <a class="menu-item" aria-label="logo" onclick={(e) => {showMenu = false; handleLogoClick()}} href="/" bind:clientHeight={headerHeight}>
    ...
    <nav style="max-height: {showMenu ? menuHeight + headerHeight : ''}px">
      ...
      <ul id="menu" bind:clientHeight={menuHeight}>
        <li class="menu-item underline" class:active={$page.url.pathname == '/cosa-facciamo' || $page.url.pathname.includes('/cosa-facciamo/')}><a href="/cosa-facciamo" onclick={(e) => {showMenu = false}} onkeydown={(e) => {if (e.key === 'Enter' || e.key === ' ') {showMenu = false}}}>Cosa facciamo</a></li>
        <li class="menu-item underline" class:active={$page.url.pathname == '/con-chi'}><a href="/con-chi" onclick={(e) => {showMenu = false}} onkeydown={(e) => {if (e.key === 'Enter' || e.key === ' ') {showMenu = false}}}>Con chi</a></li>
        <li class="menu-item underline" class:active={$page.url.pathname == '/pubblicazioni' || $page.url.pathname.includes('/pubblicazioni/')}><a href="/pubblicazioni" onclick={(e) => {showMenu = false}} onkeydown={(e) => {if (e.key === 'Enter' || e.key === ' ') {showMenu = false}}}>Pubblicazioni</a></li>
        <li class="menu-item underline" class:active={$page.url.hash == '#chi-siamo'}><a data-sveltekit-noscroll href="/#chi-siamo" onclick={(e) => {showMenu = false; handleClick}} onkeydown={(e) => {if (e.key === 'Enter' || e.key === ' ') {showMenu = false}}}>Chi siamo</a></li>
        <li class="menu-item underline" class:active={$page.url.pathname == '/archivio' || $page.url.pathname.includes('/archivio/')}><a href="/archivio" onclick={(e) => {showMenu = false}} onkeydown={(e) => {if (e.key === 'Enter' || e.key === ' ') {showMenu = false}}}>Archivio</a></li>
      </ul>
    </nav>
  </header>
  ...
  <footer class="uppercase text-xs">
    ...
    <div class="credits">
      <p>Design: <a class="underline" href="https://obelo.it/" target="_blank">studio òbelo</a></p>
      <p>Sviluppo: <a class="underline" href="https://lucabunino.com/" target="_blank">Luca Bunino</a></p>
    </div>
  </footer>

  {#if $page.state.showNewsletter}
  <div role="button" tabindex="0" id="newsletter-bg" onclick={closeNewsletter} onkeydown={(e) => {if (e.key === 'Enter' || e.key === ' ') {closeNewsletter}}}></div>
  <div id="newsletter">
    <button aria-label="newsletter-switch" id="newsletter-switch" onclick={closeNewsletter} onkeydown={(e) => {if (e.key === 'Enter' || e.key === ' ') {closeNewsletter}}}>
